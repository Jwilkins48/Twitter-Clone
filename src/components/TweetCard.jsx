  ...
  return (
    <Card>
      <div className="tweetUserInfo">
        {" "}
        <img
          className="twitterFeedImages"
          src={item.src}
          alt="user profile"
          width="50"
          height="47"
        ></img>
        <span className="users-Name">{item.name} </span> @{item.username} .{" "}
        {item.timePosted} hr
        ...
        <div className="optionsBtnContainer">
          ...
          <div className="dropdown">
            ...
            <div className={dropDown ? "dropdownActive" : "dropdownMenu"}>
              <ul className="dropdownMenuList">
                <li> {frown} Not interested in this Tweet</li>
                <li>
                  {" "}
                  {profileIcon} Follow @{item.username}
                </li>
                <li>
                  {" "}
                  {lists} Add/remove @{item.username} from Lists
                </li>
                <li>
                  {" "}
                  {mute} Mute @{item.username}
                </li>
                <li>
                  {" "}
                  {block} Block @{item.username}
                </li>
      ...
      <div className="postedTweetIcons">
        <button className="postedIcons comment">
          {comment} {item.comments}
        </button>
        <button
          style={{ color: retweetIsActive ? "#19cf86" : "" }}
          onClick={() => retweetClick()}
          className="postedIcons retweet"
        >
          {retweet} {retweetAmount}
        </button>
        <button
          style={{ color: isActive ? "#b03060" : "" }}
          onClick={() => handleClick()}
          className="postedIcons heart"
        >
          {heart} {likeAmount}
        </button>
